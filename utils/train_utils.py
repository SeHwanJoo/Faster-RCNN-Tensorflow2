...
def get_hyper_params(backbone, **kwargs):
    ...
    hyper_params["train_nms_topn"] = 2000
...
def faster_rcnn_generator(dataset, anchors, hyper_params):
    ...
    inputs:
        ...
        hyper_params = dictionary
    img: (batchsize, imagesize, imagesize, rgb)
    gt_boxes: (batchsize, 배치 사이즈내 bbox 최대개수 , 4)
    gt_labels: (batchsize, 4)
    bbox_deltas: (4, 9216, 4)
    bbox_labels: (4, 32, 32, 9)
    outputs:
...
def calculate_rpn_actual_outputs(anchors, gt_boxes, gt_labels, hyper_params):
    ...
    (feature_map_width, feature_map_height) = hyper_params["feature_map_shape"]
    ...
    # iou_map = (batch_size, feature_map_width * feature_map_height * anchor_count, bbox_count)
    # shape=(4, 9216, 6), dtype=float32)
    iou_map = bbox_utils.generate_iou_map(anchors, gt_boxes)
    # Get max index value for each row
    # shape=(4, 9216), dtype=int32)
    ...
    # shape=(4, 9216), dtype=float32)
    merged_iou_map = tf.reduce_max(iou_map, axis=2)
    # shape=(4, 9216), dtype=bool)
    pos_mask = tf.greater(merged_iou_map, 0.7)
    # shape=(4, 6), dtype=bool)
    valid_indices_cond = tf.not_equal(gt_labels, -1)
    # shape=(10, 2), dtype=int32)
    ...
    # shape=(4, 63, 38, 9), dtype=float32)
    bbox_labels = tf.reshape(bbox_labels, (batch_size, feature_map_width, feature_map_height, anchor_count))
