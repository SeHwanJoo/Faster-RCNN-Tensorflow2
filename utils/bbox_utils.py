...
def generate_anchors(hyper_params):
    ...
    #
    width_coords = tf.cast(tf.range(0, feature_map_width) / feature_map_width + (1 / feature_map_width) / 2, dtype=tf.float32)
    height_coords = tf.cast(tf.range(0, feature_map_height) / feature_map_height + (1 / feature_map_height) / 2, dtype=tf.float32)
    #
    grid_x, grid_y = tf.meshgrid(width_coords, height_coords)
